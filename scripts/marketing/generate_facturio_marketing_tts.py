...
Sortie :
  docs/marketing/pub-2026/audio/<variant_id>/segment_*.mp3
  docs/marketing/pub-2026/audio/<variant_id>/full.mp3  (avec --concat-full)
  docs/marketing/pub-2026/subtitles/<variant_id>.srt
"""
...
DEFAULT_JSON = PUB_ROOT / "storyboard.json"
DEFAULT_OUTPUT = PUB_ROOT / "audio"
SUBTITLES_DIR = PUB_ROOT / "subtitles"
...
def _write_srt(variant: dict[str, Any], variant_dir: Path, srt_path: Path, pause_ms: int) -> None:
    from pydub import AudioSegment

    blocks: list[str] = []
    cursor = 0
    idx = 1
    for seg in variant.get("segments") or []:
        text = (seg.get("text") or "").strip()
        if not text:
            continue
        seg_id = seg.get("id") or f"seg_{idx:02d}"
        mp3 = variant_dir / f"segment_{seg_id}.mp3"
        dur = len(AudioSegment.from_file(str(mp3))) if mp3.is_file() else 2500
...
def _concat_segments(parts: Iterable[Path], out_path: Path, pause_ms: int, fmt: str) -> None:
    from pydub import AudioSegment
...
async def _generate_variant(
    ...
    *,
    ...
    with tempfile.TemporaryDirectory(prefix="facturio_tts_") as tmp_dir:
        tmp = Path(tmp_dir)
        for seg in segments:
    ...
    srt_path = SUBTITLES_DIR / f"{variant_id}.srt"
    if segment_files and any(p.exists() for p in segment_files):
        _write_srt(variant, target_dir, srt_path, pause_ms)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--variant",
        default="30s-main",
        help="30s-main | 15s-teaser | ab-urgence | ab-metier | all",
    )
    parser.add_argument("--voice", default=None, help="Voix edge-tts")
    parser.add_argument("--format", choices=["mp3", "wav"], default="mp3")
    parser.add_argument("--pause-ms", type=int, default=None)
    parser.add_argument("--concat-full", action="store_true")
