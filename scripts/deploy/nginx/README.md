# Nginx reverse proxy (node12 → Raspberry Pi)

| Machine | Rôle |
|---------|------|
| **node12.lan** | Nginx public HTTPS (`facturio.danielcraft.fr`) |
| **raspberry-10.lan** | Backend `:3000`, frontend Nginx `:5173` (dist) |
...
```bash
export DEPLOY_APP_SERVER=raspberry-10.lan
...
```bash
curl -sS -o /dev/null -w "%{http_code}\n" http://raspberry-10.lan:3000/api/auth/login -X POST \
  -H "Content-Type: application/json" -d '{"email":"a@b.c","password":"x"}'
```

Sur **raspberry-10** : service `facturio` actif, Nginx local sur 5173 (`systemctl status nginx`).

## 502 sur `/api`

1. Mauvais hôte dans `proxy_pass` → doit être `raspberry-10.lan`, pas `localhost`.
2. Backend n’écoute que sur `127.0.0.1` → redémarrer après mise à jour (écoute `0.0.0.0`).
3. Pare-feu sur le Pi : autoriser node12 vers TCP 3000 et 5173.
