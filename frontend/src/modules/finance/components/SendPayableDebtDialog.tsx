...
export type SendPayableDebtPayload = {
  to: string
...
export function SendPayableDebtDialog({ open, debt, onClose, onSent }: Props) {
  ...
  const handleSend = async () => {
    ...
    setSending(true)
    try {
      const res = await payablesService.sendDebt(debt.id, {
        email: to,
      ...
      toast.success(`Reconnaissance envoyée à ${res.sentTo}`)
      await onSent?.()
      onClose()
    } catch (err: unknown) {
  ...
  return (
    <Dialog open={open} onClose={busy ? undefined : onClose} maxWidth="sm" fullWidth>
      <DialogTitle>Partager la reconnaissance de dette</DialogTitle>
      <DialogContent>
        {debt && (
          <Box>
            ...
            <Alert severity="success" sx={{ mb: 2, borderRadius: 2 }}>
              La dette est enregistrée. Envoyez-la par email au créancier ou copiez le lien public.
            </Alert>
